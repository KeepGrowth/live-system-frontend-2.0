  ...
  <transition name="slide">
    ...
    >
      ...
      <div class="relative p-6">
        ...
        <div v-if="okrs.length === 0" class="text-center py-10 text-slate-500 font-mono">
          [ 无OKR数据 ]
        </div>

        <div
          v-for="(okr, index) in sortedOkrs"
          :key="okr.id"
          ...
          <div
            ...
            <div class="grid grid-cols-2 gap-2 text-xs font-mono">
              <div class="flex justify-between border-b border-slate-700/50 pb-1 col-span-2">
                ...
                <span class="text-cyan-300/80">{{ formatDate(okr.createTimeStr) }}</span>
              </div>
              <div class="flex justify-between border-b border-slate-700/50 pb-1 col-span-2">
                <span class="text-slate-500">更新时间:</span>
                <span class="text-cyan-300/80">{{ formatDate(okr.updateTimeStr) }}</span>
              </div>
            </div>

            <!-- 隐藏字段展示 (可选，用于调试或详细查看) -->
            <div class="mt-3 flex flex-wrap gap-2 text-[10px] text-slate-600 font-mono uppercase">
              <span v-if="okr.goalId" class="bg-slate-950 px-1 py-0.5 rounded">目标:{{ okr.goalName }}</span>
              <span v-if="okr.programId" class="bg-slate-950 px-1 py-0.5 rounded">项目:{{ okr.programName }}</span>
            </div>
          </div>
        </div>
      </div>
...
// 计算属性：按时间倒序排列
const sortedOkrs = computed(() => {
  return [...props.okrs].sort((a, b) => new Date(b.createTime) - new Date(a.createTime))
})
