  ...
  <div class="min-h-screen bg-slate-950 text-slate-200 font-mono selection:bg-cyan-500 selection:text-black p-4 md:p-8">
    ...
    <div class="relative z-10 w-full space-y-6 px-4 md:px-8">
      ...
      <header
        ...
        <div>
          <h1 class="text-cyan-500 text-sm tracking-[0.2em] uppercase mb-1">目标档案</h1>
        ...
        <div class="mt-4 md:mt-0 flex items-center space-x-2">
          ...
          <div
            ...
            <span
              :class="getStatusLabel(goal.goalStatus).color"
              class="relative z-10"
            >{{ getStatusText(goal.goalStatus) }}</span>
          </div>
          <span class="text-slate-500 text-xs font-mono">目标ID: {{ goal.id }}</span>
        </div>
      </header>
      <!-- 附件区域 -->
      <div v-if="goal?.imageUrls?.length>0">
        <swiper-component
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div class="lg:col-span-4 space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-slate-900/80 p-5 border border-slate-800 relative">
              ...
              <el-rate v-model="goal.satisfactionScore" disabled></el-rate>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <indicator-card
              :title="'此目标关联项目数'"
              :subtitle="''"
              :value="goal?.programList?.length"
              :target-value="goal?.programList?.length"
            />
            <indicator-card
              :title="'此目标花费时间'"
              :subtitle="'小时'"
        ...
        <div class="lg:col-span-1">
          ...
          <div @click="isUserModalOpen=true"
            ...
            <span class="text-xs text-slate-200 uppercase">目标发起人</span>
            <span
          ...
          <div class="bg-slate-900 border border-slate-800 p-6 relative">
            ...
            <ul
              ...
              <li class="relative pl-8">
                ...
                <p class="text-xs text-slate-500 uppercase">目标开始时间</p>
                <p class="text-sm text-cyan-100 font-medium">{{ formatDate(goal.startDate) }}</p>
              </li>

              <!-- 目标时间线 -->
              <li class="cursor-pointer relative pl-8" v-for="(item,index) in goal.programList" :key="item.id">
                <span class="absolute -left-1 top-1.5 flex h-3 w-3">
                  <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-cyan-400 opacity-75"
                        v-if="index === goal.programList.length - 1"></span>
                  <span class="relative inline-flex rounded-full h-3 w-3 border border-slate-500 bg-slate-900"></span>
                </span>
                <p class="text-xs text-slate-500 uppercase">{{ item.programName }}</p>
                <p class="text-sm text-cyan-100 font-medium">{{ formatDate(item.estimateFinishTime) }}</p>
              </li>
