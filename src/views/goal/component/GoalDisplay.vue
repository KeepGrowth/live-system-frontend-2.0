    ...
    >
      ...
      <div class="relative overflow-hidden rounded-sm bg-slate-900/90 p-8">
        ...
        <div class="mb-6 flex justify-between border-b border-slate-700 pb-4">
          <div>
            <h2 class="font-mono text-2xl font-bold uppercase tracking-widest text-white">
              <span class="text-cyan-400">目标</span> 详情 // {{ goalData.goalName }}
            </h2>
            <p class="mt-1 text-xs font-mono text-slate-500">ID: {{ goalData.id }} | PROG: {{ goalData.goalId
              }}</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div class="space-y-5">
            <!-- 用户ID -->
            <div class="cursor-pointer group relative" @click="displayUser(goalData.user.id)">
              ...
              <div class="flex items-center">
                <span
                  :class="[
                    ...
                    getStatusColor(goalData.goalStatus)
                  ]"
                >
                  {{ getStatusText(goalData.goalStatus) }}
                </span>
                <div class="ml-3 h-1 flex-1 bg-slate-800">
                  <div class="h-full bg-cyan-500" :style="{ width: getProgressWidth(goalData.goalStatus) }"></div>
                </div>
              </div>
            </div>
            ...
            <div class="group relative">
              ...
              <div
                ...
                {{ formatDate(goalData.createTimeStr) }}
              </div>
            </div>
          </div>
          ...
          <div class="space-y-5">
            ...
            <div class="group relative">
              ...
              <div
                ...
                {{ goalData.description || '暂无详细描述' }}
              </div>
            </div>

            <!-- 更新时间 -->
            <div class="text-right">
              <p class="font-mono text-[10px] text-slate-600">LAST UPDATE: {{ formatDate(goalData.updateTimeStr)
                }}</p>
            </div>
          </div>
        </div>
