...
const handleSubmit = async () => {
  // 判断是添加请求还是编辑请求
  if (!form.value.id) {
    await goalCateStore.addGoalCate(form.value)
  } else {
    await goalCateStore.updateGoalCate(form.value)
  }
  // 关闭对话框
...
<template>
  <!--添加与删除的对话框-->
  <el-dialog v-model="visible" :title="form?.id?'编辑分类':'新增分类'" width="500">
    <el-form :model="form">
      <el-form-item label="分类名称" label-width="auto" prop="category_name">
        <el-input v-model="form.categoryName" autocomplete="off" />
      </el-form-item>
      <el-form-item label="分类描述" label-width="auto" prop="description">
