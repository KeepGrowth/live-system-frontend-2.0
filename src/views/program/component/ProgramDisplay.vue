    ...
    >
      ...
      <div class="relative overflow-hidden rounded-sm bg-slate-900/90 p-8">
        ...
        <div class="mb-6 flex justify-between border-b border-slate-700 pb-4">
          <div>
            <h2 class="font-mono text-2xl font-bold uppercase tracking-widest text-white">
              <span class="text-cyan-400">项目</span> 详情 // {{ programData.programName }}
            </h2>
            <p class="mt-1 text-xs font-mono text-slate-500">ID: {{ programData.id }} | PROG: {{ programData.programId
              }}</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div class="space-y-5">
            <!-- 用户ID -->
            <div class="cursor-pointer group relative" @click="displayUser(programData.user.id)">
              <label class="mb-1 block font-mono text-xs uppercase text-cyan-500/80">负责人</label>
              <div
                ...
                {{ programData?.user?.nickname }}
              </div>
            </div>
            <div class="group relative">
              <label class="mb-1 block font-mono text-xs uppercase text-cyan-500/80">关联目标</label>
              <div
                @click="openGoalModal(programData.goalId)"
                class="cursor-pointer rounded border border-slate-700 bg-slate-950 px-3 py-2 font-mono text-sm text-slate-300 shadow-inner group-focus-within:border-cyan-400 group-focus-within:shadow-[0_0_10px_rgba(34,211,238,0.2)]">
                {{ programData.goalName }}
              </div>
            </div>
              ...
              <div class="flex items-center">
                <span
                  :class="[
                    ...
                    getStatusColor(programData.programStatus)
                  ]"
                >
                  {{ getStatusText(programData.programStatus) }}
                </span>
                <div class="ml-3 h-1 flex-1 bg-slate-800">
                  <div class="h-full bg-cyan-500" :style="{ width: getProgressWidth(programData.programStatus) }"></div>
                </div>
              </div>
            </div>
            ...
            <div class="group relative">
              ...
              <div
                ...
                {{ formatDate(programData.createTimeStr) }}
              </div>
            </div>
          </div>
          ...
          <div class="space-y-5">
            <!-- 关键结果名称 -->
            <div class="group relative">
              <label class="mb-1 block font-mono text-xs uppercase text-fuchsia-500/80">开始时间</label>
              <h3
                ...
                {{ programData.estimateStartTime }}
              </h3>
            </div>
            <div class="group relative">
              <label class="mb-1 block font-mono text-xs uppercase text-fuchsia-500/80">截止时间</label>
              <h3
                class="rounded border border-slate-700 bg-slate-950/50 px-3 py-2 text-lg font-bold text-white shadow-[inset_0_0_20px_rgba(217,70,239,0.1)]">
                {{ programData.estimateFinishTime }}
              </h3>
            </div>
            ...
            <div class="group relative">
              ...
              <div
                ...
                {{ programData.programDesc || '暂无详细描述' }}
              </div>
            </div>

            <!-- 更新时间 -->
            <div class="text-right">
              <p class="font-mono text-[10px] text-slate-600">LAST UPDATE: {{ formatDate(programData.updateTimeStr)
                }}</p>
            </div>
          </div>
        </div>
...
// 定义 Emits
const emit = defineEmits(['update:modelValue', 'close', 'openGoalModal'])
...
// 为了模板中使用方便，做一个别名
const programData = ref(props.data)

// 目标模态框
const openGoalModal = async (goalId) => {
  emit('openGoalModal', goalId)
}

watch(
  () => props.data, (newVal) => {
    programData.value = newVal
  }, {
    deep: true
  }
)
