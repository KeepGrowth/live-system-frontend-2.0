<template>
  <div
    ...
    <!-- 1. 导航栏 -->
    <nav class="sticky top-0 z-50 w-full border-b border-cyan-500/30 bg-slate-900/80 backdrop-blur-md shadow-[0_0_15px_rgba(6,182,212,0.3)]">
      <div class="container mx-auto px-6 py-4 flex justify-between items-center">

        <!-- Logo (保持不变) -->
        <div class="flex items-center gap-2 group cursor-pointer" @click="goToBackend">
          <div class="w-8 h-8 bg-fuchsia-600 skew-x-[-12deg] flex items-center justify-center group-hover:shadow-[0_0_15px_rgba(192,38,211,0.8)] transition-all duration-300">
            <span class="text-white font-bold text-xl skew-x-[12deg]">S</span>
          </div>
          <span class="text-2xl font-black tracking-wider italic text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-fuchsia-500">
    ...
    <div class="hidden md:flex gap-8 font-mono text-sm tracking-widest items-center">
      ...
      <div class="relative group">
        <!-- 触发按钮 -->
        <button class="cursor-pointer flex items-center gap-1 text-slate-300 hover:text-fuchsia-400 transition-all focus:outline-none">
          <span>事业</span>
          <!-- 小箭头图标，增加视觉提示 -->
          <svg class="w-3 h-3 transform group-hover:rotate-180 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="absolute left-0 mt-2 w-40 bg-slate-900/95 border border-cyan-500/30 rounded-md shadow-[0_0_20px_rgba(6,182,212,0.2)] backdrop-blur-xl overflow-hidden invisible opacity-0 group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-left z-50">
          <div class="py-1">
            <router-link to="/todo"
      ...
      <div class="relative group">
        ...
        <button
          @click="goToBackend"
          class="cursor-pointer relative px-6 py-2 bg-cyan-900/30 border border-cyan-500 text-cyan-400 font-mono text-sm hover:bg-cyan-500 hover:text-slate-900 transition-all duration-300 clip-path-button group">
          管理后台
        </button>
